...
# 대안 — Apache 없이 webpack-dev-server 로 (정적+프록시 대행) + 타입체크
...
**⚠️ 백엔드(4000)는 API 전용 — 정적 파일 서빙 안 함**(`express.static` 없음). 정적(HTML·`dist/`·assets)은
**Apache 가 서빙**하고 `/api/*` 만 4000 으로 프록시(동일출처). 운영 `codexa.kro.kr`·개발 `codexa.test`
모두 이 구조라 `npm run dev` 가 운영과 같은 형상. `localhost:4000/` 직접 열면 404 가 정상.
