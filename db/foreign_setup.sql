...
-- 2) 해외법령 메모 — 운영자 큐레이션(전역). 회원 DB(ldb_auth)에 둔다.
--    작성=운영자(adminGuard), 열람=전체 공개. 회원별이 아니라 법조문(seg)별 1건.
--    seg-level 적재(STN)에 맞춰 메모 키 = 안정 논리키 (law_code, article_no, seg_index).
--    seg_index = article 내 1-based 순위 → 재적재(물리 id 변경)·이관에도 메모 보존.
--    ※ 기존(개인별) 테이블에서 전환하려면 db/foreign_memo_global.sql(데이터 보존).
USE ldb_auth;
...
CREATE TABLE foreign_memo (
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uq_seg (law_code, article_no, seg_index),
